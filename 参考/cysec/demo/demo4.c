...
static int tls_read_timeout(TLS_CLIENT_PCTX ctx, unsigned char *buf, size_t buflen, int *errcode, int fd, long timeout)
{
	struct timeval tv;
	fd_set read_fds;
	int ret = -1;
	...
	return tls_read(ctx, buf, buflen, errcode);
}
...
int main(int argc, char **argv)
{
	...
	int rc = 0;
	int errorcode = 0;
	size_t sent_counts = 0,read_counts = 0;
	long timeout = 5;
  ...
  if(!host||!capath||port == 0)
	...
	while(1)
	{
			...
			{
				/** 读数据 */
				rc = tls_read_timeout(tls_ctx, (unsigned char *)read + read_counts, sizeof(message) - read_counts, &errorcode , sockfd, timeout);
				/** 如果读到数据 ,则继续读 */
				if(rc > 0 && rc < (int)(sizeof(read) - read_counts )){
					printf("receviced:(%s)(%d)\n", (char *)read + read_counts, rc);
				...
				/** 超出BUF大小 跳出 */
				else if(rc > 0 && (rc == (int)(sizeof(read) - read_counts)))
					break;
				/** 重读 */
