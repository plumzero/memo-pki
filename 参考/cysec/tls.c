...
/**
	...
	先开一个
	openssl s_server -cert rsa.ssl.server.crt.pem -key rsa.ssl.server.pvk.pem -port 443 -state -CAfile /etc/pki/CA/cacert.pem
	openssl s_server -cert rsa.ssl.server.crt.pem -key rsa.ssl.server.pvk.pem -ssl3 -port 445 -state -debug -CAfile /etc/pki/CA/cacert.pem
 */
...
/**
	...
	openssl s_server -cert rsa.ssl.server.crt.pem -key rsa.ssl.server.pvk.pem -ssl3 -port 444 -state -debug -CAfile /etc/pki/CA/cacert.pem
 */
...
static int test_ssl_benchmark(void)
{
	...
	for (n = 0; n < sizeof(p)/sizeof(char*); n ++) {
		test_ssl_benchmark_verify_none(srvaddr, 7443+n*2);
	}

	for (n = 0; n < sizeof(p)/sizeof(char*); n ++) {
		//ca的自签名证书
		snprintf(path, sizeof(path), "/etc/pki/CA/cacert.pem");
		rootcrt = FILE_getcrt(path);
		//服务器证书
		snprintf(path, sizeof(path), "/usr/local/ssl/%s.ssl.server.crt.pem", p[n] );
		srvcrt = FILE_getcrt(path);
		//tls客户端私钥
		snprintf(path, sizeof(path), "/usr/local/ssl/%s.ssl.client.pvk.pem", p[n]);
		pvk = FILE_getpvk(path);
		//tls客户端证书
		snprintf(path, sizeof(path), "/usr/local/ssl/%s.ssl.client.crt.pem", p[n]);
		crt = FILE_getcrt(path);
		//设置证书链
		cm = certmgr_new();
		certmgr_add_ca(cm, srvcrt);
		certmgr_add_ca(cm, rootcrt);
		//测试
		test_ssl_benchmark_verify_required(srvaddr, 7443+n*2, cm, crt, pvk);	
		//释放
		pkey_free(pvk);
		x509crt_free(srvcrt);
		x509crt_free(crt);
