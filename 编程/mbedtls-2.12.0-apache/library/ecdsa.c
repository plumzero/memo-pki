                ...
                int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    do
    {
        ...
        do
        {
            MBEDTLS_MPI_CHK( mbedtls_ecp_gen_keypair( grp, &k, &R, f_rng, p_rng ) );	//生成私钥k和公钥R
            MBEDTLS_MPI_CHK( mbedtls_mpi_mod_mpi( r, &R.X, &grp->N ) );	// r = R.X mod grp->N

            if( key_tries++ > 10 )	//尝试10次
            {
                ret = MBEDTLS_ERR_ECP_RANDOM_FAILED;
                goto cleanup;
            }
        }
        while( mbedtls_mpi_cmp_int( r, 0 ) == 0 );	//R.X与grp->N相异
        ...
        while( mbedtls_mpi_cmp_int( &t, 1 ) < 0 ||			//确保 1<t<grp->N
