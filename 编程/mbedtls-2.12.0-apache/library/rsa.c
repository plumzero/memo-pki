                                 ...
                                 size_t output_max_len)
{
     ...
     */
    bad |= *p++; /* First byte must be 0 */		//buf中第一个字节是0

    /* This test does not depend on secret data */
    if( mode == MBEDTLS_RSA_PRIVATE )
    {
        bad |= *p++ ^ MBEDTLS_RSA_CRYPT;		//buf中第二个字节是 MBEDTLS_RSA_CRYPT

        /* Get padding len, but always read till end of buffer
         * (minus one, for the 00 byte) */
        for( i = 0; i < ilen - 3; i++ )		//从缓冲区第3个字节开始校验，
        {
            pad_done  |= ((p[i] | (unsigned char)-p[i]) >> 7) ^ 1;			//确保pad_done为0
            pad_count += ((pad_done | (unsigned char)-pad_done) >> 7) ^ 1;	//pad_done只要为0，pad_count就会一真自增1个，直到遇到pad_done非0，自增停止
        }

        p += pad_count; //跳过pad_count个填充段
        bad |= *p++; /* Must be zero */	//填充段的最后一个字节是0
    }
    else
    {
